...
VOID CHardware::SetBIOSDigit()
{
    ...
    if ( IsPlatformNT() )
    {
        dwBIOSChecksum  = CalculateRegKeyChecksum( (LPSTR)"SystemBiosDate" );
        dwBIOSChecksum += CalculateRegKeyChecksum((LPSTR)"SystemBiosVersion" );
...
UINT CHardware::CalculateRegKeyChecksum(LPSTR lpszKey)
{
    ...
    lStatus = RegOpenKeyEx( HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System", 0, KEY_QUERY_VALUE, &hkSystem );
    ...
    if ( lStatus == ERROR_SUCCESS )
    {
        ...
        WCHAR lpwszKey[80];
        MultiByteToWideChar(CP_ACP, 0, lpszKey, -1, lpwszKey, 80);
        lStatus = RegQueryValueEx( hkSystem, lpwszKey, NULL, &dwValueType, Buffer, &dwBufferSize );
...
VOID CHardware::SetHDSerialDigit()
{
    ...
    wsprintf( (LPWSTR)szBootDrivePath, L"C:\\" );
    fInfoSuccess = GetVolumeInformation((LPCWSTR)szBootDrivePath, NULL, 0, &dwVolumeSerialNumber, &dwMaximumComponentLength, &dwFileSystemFlags, NULL, 0 );
...
DWORD CHardware::CalculateDriveCapacity95( INT nDrive )
{
    ...
    hDevice = CreateFile( L"\\\\.\\vwin32", 0, 0, NULL, 0, FILE_FLAG_DELETE_ON_CLOSE, NULL );
...
DWORD CHardware::CalculateDriveCapacityNT(INT nDrive)
{
    ...
    wsprintf((LPWSTR)szDrive, L"%c:", 'A' + ( nDrive - 1 ) );   // Create DOS Drive Identifier (A: or B:)

    dwBytesReturned = QueryDosDevice((LPWSTR)szDrive, (LPWSTR)szDriveAssignment, MAX_PATH );
    ...
    if ( dwBytesReturned != 0 )
    {
        ...
        lpszWalkString = (LPTSTR)szDriveAssignment;
        ...
        if ( lstrcmp((LPCWSTR)szDriveAssignment, L"\\Device" ) == 0 )
        {
            fDriveExists = TRUE;
        }
    }
    ...
    if ( fDriveExists == TRUE )
    {
        ...
        wsprintf((LPWSTR)szDrive, L"\\\\.\\%c:", 'A' + ( nDrive - 1 ) ); // Create NT Drive Identifier (\\.\A: or \\.\B:)

        hDevice = CreateFile((LPCWSTR)szDrive, 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL );
...
VOID CHardware::SetVideoBIOSDigit()
{
    ...
    if ( IsPlatformNT() )
    {
        dwVideoBIOSChecksum  = CalculateRegKeyChecksum( (LPSTR)"VideoBiosDate" );
        dwVideoBIOSChecksum += CalculateRegKeyChecksum( (LPSTR)"VideoBiosVersion" );
